...
async function SendMessage (
  ...
  respondeCallback, image=null, audio = null, isUpToDate=false) {
  ...
  const response = await fetch(MESSAGES_ENDPOINT, {
    ...
    body: JSON.stringify({
      ...
      audio : audio, // the voice uploaded by the user
...
function App() {
  ...
  const handleSubmit = (message) => {
    ...
    SendMessage(message, userID, sessionID, "", "", recieveMessage, selectedImage, selectedVoice)
  ...
  useEffect(() => {
    const response = SendMessage("", userID, sessionID, "", "", recieveMessage, null, true)
    initialMessage()
  ...
  const {
    ...
    const handleSave = async () => {
      ...
      setSelectedVoiceFile(audioFile);
      
      reader.onload = () => {      
       setSelectedVoice(reader.result)
      }
    };
